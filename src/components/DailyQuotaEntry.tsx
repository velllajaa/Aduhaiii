...
export const DailyQuotaEntry: React.FC<DailyQuotaEntryProps> = ({
  ...
  const handleActivityLevelChange = (member: Member, newLevel: ActivityLevel) => {
    ...
    let note = currentLog?.note || '';

    // Auto-update note if empty or default
    if (!note || note.includes('***No Logs***') || note.includes('***Exempted***') || note.includes('***On Leave')) {
      if (newLevel === 'under_1h') note = '***No Logs*** (Demotion notice)';
      else if (newLevel === 'exempted') note = '***Exempted***';
      else if (newLevel === 'above_3h' || newLevel === 'under_3h') note = '';
    }
            ...
            {batMembers.length === 0 ? (
              ...
              <div className="divide-y divide-zinc-800/60">
                {batMembers.map((member) => {
                  ...
                  const currentLevel: ActivityLevel = log?.activityLevel || (log?.status as any) || 'under_1h';
