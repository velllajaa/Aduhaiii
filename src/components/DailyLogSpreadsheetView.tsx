...
export const DailyLogSpreadsheetView: React.FC<DailyLogSpreadsheetViewProps> = ({
  ...
  const handleUpdateHour = (member: Member, hourValue: string) => {
    let activityLevel: ActivityLevel = 'under_1h';
    let note = '';

    if (hourValue === '3 hour' || hourValue === '4 hour+') {
      activityLevel = 'above_3h';
    } else if (hourValue === '2 hour') {
      activityLevel = 'under_3h';
    } else if (hourValue === '1 hour') {
      activityLevel = 'under_2h';
    } else if (hourValue === 'Exempted' || hourValue === 'LOA') {
            ...
            <tbody className="divide-y divide-zinc-800/80 bg-zinc-950/90 text-zinc-200">
              {filteredMembers.length === 0 ? (
                ...
                filteredMembers.map((member) => {
                  ...
                  const inGameHour = log?.gameHourDropdown || (log?.activityLevel === 'above_3h' ? '3 hour' : log?.activityLevel === 'under_3h' ? '2 hour' : log?.activityLevel === 'under_2h' ? '1 hour' : log?.activityLevel === 'exempted' ? 'Exempted' : '-');
