import { BattalionId, Member, DailyMemberLog, AppSettings, BattalionDailyReportData, ActivityLevel } from '../types';
...
export function getActivityEmoji(log?: DailyMemberLog): string {
  if (!log) return '🔴';
  const level: ActivityLevel = log.activityLevel || (log.status as any) || 'under_1h';
  return ACTIVITY_META[level]?.emoji || '🔴';
}
