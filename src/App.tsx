...
import { loadAppState, saveAppState, clearAllPersonnelData } from './utils/storage';
...
export default function App() {
  ...
  const handleUpdateMemberLog = (memberId: string, updates: Partial<DailyMemberLog>) => {
    ...
    setAppState((prev) => {
      ...
      const updatedLog: DailyMemberLog = {
        ...
        activityLevel: updates.activityLevel || updates.status as any || existing.activityLevel || 'under_1h',
        updatedAt: new Date().toISOString(),
      };
  ...
  const handleBulkUpdateStatus = (battalionId: BattalionId | 'all', newLevel: ActivityLevel) => {
    ...
    let defaultNote = '';
    if (newLevel === 'under_1h') defaultNote = '***No Logs*** (Demotion notice)';
    else if (newLevel === 'exempted') defaultNote = '***Exempted***';
    ...
    for (const m of targetMembers) {
      ...
      newLogs[key] = {
        ...
        note: defaultNote || existing?.note || '',
        updatedAt: new Date().toISOString(),
      };
    }
